...
function getHTTPConfig(req) {
  ...
  if (req.headers?.["accept"].includes("application/json")) acceptBasedHeader = {};
  else acceptBasedHeader = { responseType: "stream" };
...
app.get("/", function (req, res) {
  ...
  res.send(
    "<div style='background: #36383F; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; font-size: 3em; color: white;'>Reverse proxy working...</div>"
  );
});
...
app.get(":endpoint([\\/\\w\\.-\\?\\=]*)", async function (req, res) {
  ...
  try {
    ...
    if (backendResponse?.headers?.["content-type"].includes("application/json")) res.json(backendResponse.data);
    else {
...
app.put(":endpoint([\\/\\w\\.-\\?\\=]*)", async function (req, res) {
    let endpoint = backendAddr + baseEndPoint + req.params.endpoint;
    console.log("POST | " + getFullTime() + " | " + endpoint);
    ...
    try {
      const backendResponse = await axiosClient.put(
        baseEndPoint + req.params.endpoint + queryParams,
        req.body,
        httpConfig
      );
